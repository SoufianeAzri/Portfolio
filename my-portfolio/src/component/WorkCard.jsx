...
const WorkCard = ({ item }) => {
  return (
    <Link
      ...
      target="_blank"
      style={{
        cursor: !item.link && "default",
      }}
    >
      <div className="shadow-lg shadow-[#040c16] rounded-md mx-auto h-[390px] p-4 relative">
        <div
          style={{ backgroundImage: `url("${item.image}")` }}
          className="w-full h-[63%] bg-cover rounded-md"
        ></div>
        <div className="py-2">
          <h1 className="font-bold text-[20px]">{item.title}</h1>
          <p className="font-semibold text-[14px] text-main-gray">
            {item.descreption}
          </p>

          <div className="flex justify-between items-center pt-2">
            <ul className="flex gap-2 font-semibold text-[15px]">
              {item.tags.map((tag) => (
                <li
                  className={`${
                    tag === "Next"
                      ...
                      ? "text-yellow-500"
                      : "text-blue-800"
                  } p-0 flex gap-1 items-center`}
                  key={tag}
                >
                  {tag === "Html" ? (
                    <AiOutlineHtml5 size={18} />
                  ) : tag === "Css" ? (
                    <FaCss3 size={18} />
                  ) : tag === "React" ? (
                    <DiReact size={18} />
            ...
            <p className="text-main-gray text-sm font-bold">{item.year}</p>
          </div>
        </div>
